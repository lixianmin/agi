...
Go-based AI/LLM client SDK，为各类 LLM 供应商 API 提供 Go 客户端库。

### 目录结构

1. **chat/** — 共享领域类型（Message, Request, Thread），所有供应商共用
2. **siliconflow/** — SiliconFlow API 客户端：Chat、StreamChat、TranscribeAudio、Embeddings
3. **deepseek/** — DeepSeek API 客户端：Chat
4. **ifs/** — 共享接口和常量

### golang

1. 项目使用 vendor，依赖 vendored 到 vendor/ 目录
2. go 版本: 1.22+
...
// JSON 序列化使用 github.com/lixianmin/got/convert
bts, err := convert.ToJsonE(request)
err := convert.FromJsonE(bts, &response)
```
...
```bash
go test ./...          # 运行所有测试
go test ./siliconflow/ # 运行 siliconflow 包测试
go vet ./...           # 代码检查
...
1. 在对应供应商包的 `type (...)` 块中添加 request/response 类型
2. 在 Client struct 上添加方法，遵循现有 HTTP 调用模式
3. 在 `*_test.go` 中添加测试
